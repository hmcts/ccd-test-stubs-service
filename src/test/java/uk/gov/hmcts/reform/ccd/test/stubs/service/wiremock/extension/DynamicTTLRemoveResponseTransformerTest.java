...
class DynamicTTLRemoveResponseTransformerTest {
    ...
    @Test
    @DisplayName("Should remove TTL from response")
    void shouldRemoveTTLFromResponse() {

        // GIVEN
        Request request = mock(Request.class);
        when(request.getBodyAsString()).thenReturn("{\"data\":{\"TTL\":{}, \"OtherField\":\"value\"}}");
    ...
    void shouldNotFailIfNoTTLInRequest() {
        ...
        // GIVEN
        Request request = mock(Request.class);
        when(request.getBodyAsString()).thenReturn("{\"case_data\":{\"OtherField\":\"value\"}}");
        Response response = Response.response().body("{\"data\":{}}").build();
